...
function font_uploader_scripts()
{
...
function font_uploader_styles() {
	$fontFileDir= WP_PLUGIN_URL.'/'.str_replace(basename( __FILE__),"",plugin_basename(__FILE__));
    wp_enqueue_style("fontFunctions", $fontFileDir."/fontFunctions/fontFunctions.css", false, "1.1", "all");
}
if(isset($_GET['page']) && $_GET['page'] == 'font-uploader.php') {
	add_action('admin_print_scripts', 'font_uploader_scripts');
	add_action('admin_print_styles', 'font_uploader_styles');
}
